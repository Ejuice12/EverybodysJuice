  ...
  const renderMedia = useCallback(async () => {
    ...
    try {
      ...
      while (pending) {
        ...
        switch (result.type) {
          case "error":
            setState({ status: "error", renderId, error: new Error(result.message) });
            pending = false;
            break;
          case "done":
            setState({ size: result.size, url: result.url, status: "done" });
            pending = false;
            break;
          case "progress":
            setState({ status: "rendering", bucketName, progress: result.progress, renderId });
            await wait(1000);
        }
      }
    } catch (err) {
      setState({ status: "error", error: err as Error, renderId: null });
    }
  }, [id, inputProps]);

  const undo = useCallback(() => setState({ status: "init" }), []);
